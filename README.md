...
- Correction-model downloads are also explicit and local-only; Fix downloads can be managed on the Models screen, and QuietType now attempts experimental local SmolLM2 GGUF correction when enabled while automatically falling back to built-in cleanup if model output is empty/bad.
...
The project has an Android/Kotlin/Compose prototype with interactive Material 3 setup, settings, compact model/language profiles with Custom model management, History, and About screens. Onboarding/settings/model choices are persisted locally. The app can download and verify sherpa-onnx model archives with a Material progress bar and percentage text, unpack prepared runtime files, start a foreground local `AudioRecord` + sherpa-onnx dictation session from the floating overlay, run experimental local SmolLM2 GGUF-based Fix correction with deterministic fallback, and broadcast final transcripts back to the Accessibility service for focused-field insertion. Successful final dictations can optionally be saved to local-only history for later copying/deletion. The Models screen starts explicit HTTPS model downloads, verifies SHA-256 before storing a downloaded/prepared marker, deletes private model files, and now includes a separate correction-model section where selecting a downloadable Fix model immediately starts its download and offers deletion afterward. The default Danish/multilingual profile uses a locked Parakeet TDT v3 int8 archive with a real GitHub release URL and SHA-256 checksum; alternative compact multilingual and English low-latency profiles can switch to smaller/faster candidate models and auto-start their downloads; Custom reveals all ASR options including Whisper tiny/base candidates with corrected checksums. The floating overlay now also shows immediate `✨ Fixing` feedback when Fix is pressed. It is still not public-release-ready: Danish live insertion needs chunked offline recognition, and model-based correction needs real device-level quality/performance validation before release. A local-only `ROADMAP.md` file may exist in developer checkouts and is intentionally ignored by git. Tracked planning and release documents live under `docs/`.
